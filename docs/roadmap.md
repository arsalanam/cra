...
### User administration (Sprints U1+U2+U3+U4 ALL SHIPPED 2026-06-01)

The platform's existing invite endpoint (`/api/admin/users`) was bare — global-scoped flat role list, no profile capture, no separation-of-duties enforcement, no delegation log. U1 introduced a regulatory-aware admin surface that wraps Cognito invite + enforces ICH E6 + Part 11 expectations at grant time. U2 layered the admin UI. U3 added the user-side onboarding wizard + clinical-write enforcement gate. U4 closes the auditor loop with audit log + PI countersigning + 3 PDF reports + expiry alerts + suspension enforcement.
...
- **Sprint U3 — onboarding flow (`/onboarding.html`) + enforcement gate.** ✅ Shipped 2026-06-01. Post-login wizard at `/onboarding.html` with a conditional stepper (Welcome → Identity → Licensure → Training → Disclosure → Delegation → Complete; only the steps that the user's roles require are shown). Backend `/api/onboarding/*` router with 5 endpoints (GET `/me`, PATCH `/me/profile`, POST `/me/training-records`, POST `/me/delegation-entries`, POST `/me/complete`). `/auth/me` extended with `onboarding_required` + `onboarding_completed_at` + `missing_fields_by_role` so the frontend knows when to redirect. Server-side ENFORCEMENT: `services/user_admin.CLINICAL_WRITE_PERMS` (37 perms covering eCRF capture / safety / drug / lab / source-doc / signing / lock / randomisation) + `require_permission_scoped` extended to also 403 with `"Onboarding required"` when the perm is in that set AND the caller's `UserProfile.onboarding_completed_at` is unset. Read-only + research-tier perms are NOT gated. Hard redirect on collector.html + ecrf.html via inline IIFE; soft banner on index.html main chat. GCP training upload mirrors the date/provider/URL into the profile so the same field doesn't need re-entry. Delegation entries are user-captured-unsigned; PI countersigns from admin UI (U4). Auditor independence captured as `TrainingRecord(training_type=other, topic=auditor_independence)`. POST `/me/complete` re-validates EVERY required field on the server before setting `onboarding_completed_at`. 23 new tests (1565 total). Updated 6 integration-test `_seed_user` helpers to pre-onboard test users so existing clinical-write tests still pass.
- **Sprint U4 — audit + reports + PI countersigning + suspension gate.** ✅ Shipped 2026-06-01. New `UserAdminAuditEntry` append-only table + `services/user_admin_audit.py` with 15 canonical action constants (`invite.created`, `role.granted`, `role.grant_overridden`, `user.suspended`, `delegation.signed`, etc.) + `record_event` helper. Audit recording hooks landed at every state-changing endpoint (invite, resend, revoke, grant, revoke, suspend, reactivate, profile patch, training upload, onboarding complete). New endpoints: `POST /delegation-entries/{id}/sign` (PI countersigning with Part 11 §11.200 password reauth + PI-on-trial authority check), `GET /delegation-queue` (unsigned entries on trials the caller is PI on, global PI sees all), `GET /training-expiring?within_days=` (training records expiring within N days), `GET /audit` (filtered audit-log query with actor/target/action/from/to + 1000-row cap), `GET /audit/actions` (canonical action enum for the filter dropdown). Three new PDF reports: `GET /reports/delegation-log/{trial_id}.pdf` (ICH E6 §4.1.5 format), `GET /reports/training-matrix/{site_id}.pdf` (ICH E6 §4.2.4 with EXPIRED/DUE SOON colour-coding), `GET /reports/audit-log.pdf` (Part 11 §11.10(e) inspector view); each reuses the shared report styles. `require_permission_scoped` extended again — now ALSO 403s when `UserProfile.suspended_at` is set (applies to EVERY perm check, not just clinical-writes — distinct from the onboarding gate); suspension error wins when both fire. `/users-admin.html` extended with 4 new tabs (Audit log / PI sign queue / Training expiring / Reports) sharing the existing filter strip pattern, plus a password-reauth modal for delegation signing. 21 new tests (1586 total). Auditor-ready exports for inspection use.
