...
| Analysis | CDISC SDTM mapping → ADaM → TLF | **P0** | ✅ shipped 2026-05-30 (7 SDTM domains + ADSL + ADTTE + K-M / Cox PH; define.xml / XPT deferred) | XL |
...
- **Deferred for follow-up:** `define.xml` (today: `define-overview.txt` plain text); SAS Transport (XPT) — CSV is regulator-acceptable for many flows but XPT is the submission default; real MedDRA SOC back-indexing (PT is captured free-text); WHODrug for CM (CMDECOD free-text today); randomisation-derived TRT01P/TRT01A (waiting on IRT, P0 above); additional ADTTE parameters beyond TTAE/TTSAE/DEATH (the architecture supports them — add to `_PARAMS` in `derive_adtte`).
