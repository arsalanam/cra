...
- **Sprint U2 — admin UI (`/users-admin.html`).** Searchable user list scoped to Account / Trial / Study / Site, guided role grant with regulatory tooltips from the roles catalogue, client-side SoD warnings via `POST /check-grant` pre-flight, bulk-create from CSV, resend / revoke / suspend buttons. **Benefit:** admin doesn't need to read the rbac-design doc to invite a coordinator + assign them to a site + sign off on their training.
