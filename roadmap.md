...
### Account layer (Sprints A1 + A2 + A2.5 shipped 2026-06-01)

The structural layer (Account → ClinicalTrial → AccountSite, with TrialSite + AccountMember joins) shipped 2026-06-01. A2 wired the new layer into existing flows; A2.5 closed the chat-handoff loop so threads auto-bind to Trial artefact slots end-to-end. Only A3 (resumability audit across Phase-04 demos) remains.
