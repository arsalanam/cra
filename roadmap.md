...
- **Sprint A2 — wire Account / Trial context into existing flows.** ✅ Shipped 2026-06-01 (partial — core wiring landed; chat-side handoff seeding deferred). What landed: cross-store resolver (`services/trial_context.py` walks Deployment → Study → Trial → Account); auto-status hooks on the 3 lifecycle events (`design → draft` on EcrfStudy form publish, `draft → deployed` on StudyDeployment create, `deployed → locked` on study lock, `locked → deployed` on unlock) — never regresses, never touches archived; `POST /api/ecrf/studies` accepts optional `trial_id` + falls back to a fresh Trial under the Default Account; `GET /api/deployments/{id}/trial` cross-store resolver endpoint; `POST /api/accounts/{id}/transfer-ownership` (promotes new owner + demotes previous to 'admin'); `POST /api/trials/{id}/artefacts` binds a Thread to one of 5 artefact slots; collector.html shows the current Trial badge above the deployment selector with status pill + sponsor + indication + deep-link to /accounts.html. **Deferred to A2.5:** chat handoff seeds carrying `trial_id` automatically (so a manuscript thread spawned from a meta-analysis terminal card auto-binds to the same Trial via `manuscript_thread_id`); trial-aware "Save to trial" button on terminal cards.
