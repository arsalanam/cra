...
Key modules:
- `agent/dispatcher.py` — `classify_route(user_message, current_workflow)` is the routing brain; returns a `Route` (workflow, rule, sticky). Continuation prefixes are scoped to the pinned workflow; cross-workflow jumps go through `_HANDOFF_SEEDS`; definitional questions detour to general_qa without re-pinning the thread; unauthorized keyword-inferred routes fall back to general_qa (slash commands still 403). No-keyword first turns get one structured LLM call (`agent/fallback_classifier.py`, fail-open, `DISPATCHER_LLM_FALLBACK_ENABLED` to disable). See `docs/agent-loop-review.md`.
