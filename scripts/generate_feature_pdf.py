...
def build_hero(path: Path) -> None:
    ...
    for y, (_name, rr, lo, hi) in zip(y_positions, studies, strict=True):
        ...
        ax.scatter(
            [rr], [y], s=110, marker="s", color="#2A9D8F",
    ...
    diamond = plt.Polygon(
        ...
        closed=True, facecolor="#F4A261", edgecolor="white", linewidth=1.5, zorder=4,
    )
    ...
    ax.text(
        ...
        color="#CFE2EC", fontsize=10, alpha=0.85,
    )
...
styles = {
    ...
    "Tagline": ParagraphStyle(
        ...
        textColor=colors.HexColor("#CFE2EC"), alignment=TA_LEFT, spaceAfter=0,
    ),
    "EyebrowLight": ParagraphStyle(
        "EyebrowLight", parent=base["Normal"],
        fontName="Helvetica-Bold", fontSize=10, leading=12,
        textColor=colors.HexColor("#9CC9DA"), alignment=TA_LEFT, spaceAfter=6,
    ),
    "H1": ParagraphStyle(
...
def workflow_card(num: str, title: str, body: str, outcome: str) -> KeepTogether:
    num_para = Paragraph(
        f'<font color="#2A9D8F" size="20"><b>{num}</b></font>', styles["H3"],
    )
...
def differentiator_table(rows: list[tuple[str, str]]) -> Table:
    ...
    for d, w in rows:
        data.append([Paragraph(f"<b>{d}</b>", styles["BodyTight"]),
                     Paragraph(w, styles["BodyTight"])])
...
def build_pdf() -> None:
    ...
    cover_frame = Frame(
        0.6 * inch, 0.6 * inch,
        w - 1.2 * inch, h - 1.2 * inch,
        id="cover", showBoundary=0,
    )
    content_frame = Frame(
        0.6 * inch, 0.7 * inch,
        w - 1.2 * inch, h - 1.5 * inch,
    ...
    for name, why in levers:
        lever_data.append([Paragraph(f"<b>{name}</b>", styles["BodyTight"]),
                           Paragraph(why, styles["BodyTight"])])
